...
int main(int argc, const char *argv[])
{
    ...
    int dataBufferSize = 2;       // no. of images which are held in memory (ring buffer) at the same time
    std::deque<DataFrame> dataBuffer; // list 
    ...
    for (size_t imgIndex = 0; imgIndex <= imgEndIndex - imgStartIndex; imgIndex+=imgStepWidth)
    {
        ...
        clusterLidarWithROI((dataBuffer.end()-1)->boundingBoxes, (dataBuffer.end() - 1)->lidarPoints, shrinkFactor, P_rect_00, R_rect_00, RT);
         cout << "lidar points size in ROI : " <<(dataBuffer.end()-1)->lidarPoints.size() << endl;
        // Visualize 3D objects
        bVis = false;
        if(bVis)
        {
            show3DObjects((dataBuffer.end()-1)->boundingBoxes, cv::Size(4.0, 20.0), cv::Size(2000, 2000), true); // 4.0, 20.0 & (2000 2000)
        }
        bVis = false;
       ...
       myfile << descriptorType.c_str() << ",";
        ...
        if (dataBuffer.size() > 1) // wait until at least two images have been processed
        {
            ...
            bVis = true;
                            if (bVis)
                            {
                                ...
                                for (int i=0; i<7; i++)
                                {
                                    ...
                                    char str[2];
                                    sprintf(str, "%d", i);
                                    putText(visImg, str, cv::Point2f((dataBuffer.end() - 1)->boundingBoxes[i].roi.x, (dataBuffer.end() - 1)->boundingBoxes[i].roi.y), cv::FONT_HERSHEY_PLAIN, 2, cv::Scalar(0,255,0));
                                }

                                for (int i=0; i<7; i++)
                                {
                                    cv::rectangle(visImg, cv::Point((dataBuffer.end() - 2)->boundingBoxes[i].roi.x, (dataBuffer.end() - 2)->boundingBoxes[i].roi.y), cv::Point((dataBuffer.end() - 2)->boundingBoxes[i].roi.x + (dataBuffer.end() - 2)->boundingBoxes[i].roi.width, (dataBuffer.end() - 2)->boundingBoxes[i].roi.y + (dataBuffer.end() - 2)->boundingBoxes[i].roi.height), cv::Scalar(0, 100, 255), 2);
                                    char str[2];
                                    sprintf(str, "%d", i);
                                    putText(visImg, str, cv::Point2f((dataBuffer.end() - 2)->boundingBoxes[i].roi.x + (dataBuffer.end() - 2)->boundingBoxes[i].roi.width, (dataBuffer.end() - 2)->boundingBoxes[i].roi.y + (dataBuffer.end() - 2)->boundingBoxes[i].roi.height), cv::FONT_HERSHEY_PLAIN, 2, cv::Scalar(0,100,255));
                                }


                                string windowName = "BoundingBoxes";
            ...
            for (auto it1 = (dataBuffer.end() - 1)->bbMatches.begin(); it1 != (dataBuffer.end() - 1)->bbMatches.end(); ++it1)
            {
                ...
                {
                    cout <<" we have lidar pointss" <<endl;
                    //// STUDENT ASSIGNMENT
