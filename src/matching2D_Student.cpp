                      ...
                      std::vector<cv::DMatch> &matches, std::string descriptorType, std::string matcherType, std::string selectorType)
{
    ...
    { // k nearest neighbors (k=2)
        ...
        matcher->knnMatch(descSource,descRef, matches_, k);
        double minDistRatio= 0.8;
        for( auto it : matches_)
        {
            if (it.size() == 2 && (it[0].distance < minDistRatio * it[1].distance))
            {
...
void detKeypointsHarris(std::vector<cv::KeyPoint> &keypoints, cv::Mat &img, bool bVis)
{
    ...
    cv::cornerHarris(img, dst,blockSize, aperturSize, k, cv::BORDER_DEFAULT);
    ...
    for (size_t j = 0; j < dst_norm.rows; j++) {
        for (size_t i = 0; i < dst_norm.cols; i++) {
            ...
            newKeyPoint.size = 2 * aperturSize;
    ...
    t = ((double)cv::getTickCount() - t) / cv::getTickFrequency();
    cout << "Haris detection with n=" << keypoints.size() << " keypoints in " << 1000 * t / 1.0 << " ms" << endl;
